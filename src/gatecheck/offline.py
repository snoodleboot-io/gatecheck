...
A single environment variable, ``GATECHECK_OFFLINE``, marks a run as air-gapped: no
...
hook marker). ``gatecheck run --offline`` sets it for the process; the variable alone
works too.
"""
...
OFFLINE_ENV = "GATECHECK_OFFLINE"


def is_offline(environ: Mapping[str, str] | None = None) -> bool:
    """True when ``GATECHECK_OFFLINE`` is set to a non-empty value in ``environ``."""
