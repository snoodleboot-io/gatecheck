...
When no ``--config`` is given, gatecheck searches upward from the working directory,
like every other tool in this space. Each directory is checked for a ``check.toml``
first, then a ``pyproject.toml`` carrying a ``[tool.gatecheck]`` table. The walk stops
...
def discover_config(start: Path) -> Path | None:
    """Search ``start`` and its parents for a gatecheck config; return it or ``None``.

    At each directory, a ``check.toml`` wins over a ``pyproject.toml`` with a
    ``[tool.gatecheck]`` table. The search stops after a directory containing ``.git``.
    """
    for directory in _walk_up(start):
        ...
        if pyproject.is_file() and _has_gatecheck_table(pyproject):
            return pyproject
...
def _has_gatecheck_table(pyproject: Path) -> bool:
    """True when ``pyproject`` parses and contains a ``[tool.gatecheck]`` table.

    A malformed or unreadable ``pyproject.toml`` is treated as "not a gatecheck config"
    rather than an error — discovery keeps walking. If the user meant to configure
    gatecheck there, ``load_config`` reports the parse error when it reads the file.
    """
    try:
        ...
        return False
    tool = data.get("tool")
    return isinstance(tool, dict) and isinstance(tool.get("gatecheck"), dict)
