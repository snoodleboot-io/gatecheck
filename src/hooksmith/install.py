...
Writes a git hook script per event that runs ``hooksmith run <group>`` for every
group with a matching ``on-event``. Only hooksmith-managed hooks (carrying a marker
...
from hooksmith.runner import SubprocessGitClient

_MARKER = "# hooksmith-managed"
...
def install_hooks(
    config: HooksmithConfig, *, locator: HooksLocator | None = None
    ...
    existing hook is overwritten only if it is hooksmith-managed; otherwise it is
    ...
    for git_hook, groups in by_hook.items():
        ...
        if path.exists() and _MARKER not in path.read_text(encoding="utf-8"):
            outcomes.append(
                InstallOutcome(
                    git_hook, groups, "skipped", "existing hook is not hooksmith-managed"
                )
            )
            continue
...
def _groups_by_hook(config: HooksmithConfig) -> dict[str, tuple[str, ...]]:
...
def _write_hook(path: Path, git_hook: str, groups: Iterable[str]) -> None:
    """Write an executable hooksmith-managed hook script running each group.

    The ``commit-msg`` hook forwards git's ``$1`` (the message-file path) to
    ``hooksmith run --commit-msg-file`` so the message-check hooks can read it.
    """
    suffix = ' --commit-msg-file "$1"' if git_hook == _COMMIT_MSG_HOOK else ""
    lines = ["#!/bin/sh", _MARKER, "set -e"]
    lines += [f"hooksmith run {group}{suffix}" for group in groups]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    path.chmod(path.stat().st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)


def has_on_event_groups(config: HooksmithConfig) -> bool:
