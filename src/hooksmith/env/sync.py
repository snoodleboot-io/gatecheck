...
first ``hooksmith run`` is fast. Uses ``EnvManager.explain`` to classify each hook
...
from hooksmith.env.manager import EnvManager
...
def sync_environments(
    config: HooksmithConfig, *, env_manager: EnvManager | None = None
) -> tuple[SyncOutcome, ...]:
