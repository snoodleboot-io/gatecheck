...
``RegistryClient`` seam in ``hooksmith.registry``) so ``EnvManager``'s pypi branch
...
from hooksmith.env.env_cache import default_cache_root
from hooksmith.env.uv_bootstrap import bootstrap_uv
...
class SubprocessUvRunner:
    ...
    def _install(self, uv: str, pinned: ResolvedPyPISource, python: Path) -> None:
        ...
        tree. hooksmith does **not** use ``--require-hashes``: that mode requires every
        package in the resolution — including transitive dependencies — to be pinned
        and hashed, but hooksmith only knows the top-level package's version, so it
    ...
    def _find_uv(self) -> str:
        """Locate ``uv``: ``HOOKSMITH_UV`` override → ``PATH`` → auto-bootstrap a pinned uv.
        ...
        ``allow_bootstrap`` is false or ``HOOKSMITH_NO_BOOTSTRAP`` is set.
        """
        env = os.environ if self._environ is None else self._environ
        override = env.get("HOOKSMITH_UV")
        if override:
            candidate = Path(override)
            if candidate.is_file() and os.access(candidate, os.X_OK):
                return str(candidate)
            raise UvNotFound(f"HOOKSMITH_UV={override!r} is not an executable file")
        located = shutil.which("uv", path=env.get("PATH", os.defpath))
        if located is not None:
            return located
        if not self._allow_bootstrap or env.get("HOOKSMITH_NO_BOOTSTRAP"):
            raise UvNotFound(
                "uv not found on PATH (set HOOKSMITH_UV, install uv, or allow "
                "auto-bootstrap by unsetting HOOKSMITH_NO_BOOTSTRAP)"
            )
