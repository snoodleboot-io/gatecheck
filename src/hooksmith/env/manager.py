...
from hooksmith.env.env_cache import default_cache_root, is_healthy, publish_atomically, venv_slot
from hooksmith.env.env_error import EnvError
from hooksmith.env.uv_bootstrap import UvBootstrapError
from hooksmith.env.uv_runner import SubprocessUvRunner, UvBuildError, UvNotFound, UvRunner
from hooksmith.offline import is_offline
from hooksmith.registry import RegistryClient, ResolvedPyPISource, resolve_pypi_source
    ...
    def _resolve_pypi(self, hook: HookDef, source: PyPISource, tool: str) -> ResolvedEnv:
        ...
        if self._offline and not is_healthy(venv_slot(cache_root, key)):
            raise EnvError(
                ...
                "run 'hooksmith sync' while online first to warm the cache",
            )
        runner = (
            self._uv_runner
        ...
        try:
            ...
            raise EnvError(
                ...
                "(set HOOKSMITH_UV, install uv, or allow auto-bootstrap)",
            ) from exc
