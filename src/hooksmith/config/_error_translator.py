...
from hooksmith.sources import SourceSpecError, parse_source
...
def _walk_doc(
    ...
    ``table_prefix`` (e.g. ``"tool.hooksmith"``) is prepended when the config lives in
...
def _prefixed(table_prefix: str, name: str) -> str:
    """``name`` qualified by ``table_prefix`` (``tool.hooksmith.hook``); ``name`` if no prefix."""
