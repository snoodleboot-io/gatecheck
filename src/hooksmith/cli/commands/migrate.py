"""`hooksmith migrate` — convert a .pre-commit-config.yaml into check.toml."""
...
from hooksmith.migration import MigrationError, map_precommit, parse_precommit_config
...
def migrate(input_path: Path, output_path: Path) -> None:
    """Translate a pre-commit config into a hooksmith check.toml (best-effort)."""
    try:
