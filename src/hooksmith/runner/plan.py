...
Turns a ``HooksmithConfig`` and a run context into a validated, dependency-ordered
...
from hooksmith.config import HookDef, HooksmithConfig
from hooksmith.offline import is_offline
from hooksmith.runner.changeset import route_files
...
def _select(
    config: HooksmithConfig, group: str | None, by_id: Mapping[str, HookDef]
) -> list[HookDef]:
