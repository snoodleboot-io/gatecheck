...
| Artifact | State |
|---|---|
| [src/hooksmith/config/schema.py](../../src/hooksmith/config/schema.py) | Stub — empty pydantic classes |
| [src/hooksmith/config/loader.py](../../src/hooksmith/config/loader.py) | Stub — `raise NotImplementedError` |
| [src/hooksmith/config/__init__.py](../../src/hooksmith/config/__init__.py) | Re-exports the stubs |
...
| PRD / ADR / Feature / Story | [PRD-0001](../prd/0001-hooksmith.md), [ADR-0001](../adr/0001-python-host-rust-core.md), [FEAT-0001](../features/FEAT-0001-config-loader/feature.md), [STY-0001](../features/FEAT-0001-config-loader/stories/STY-0001-load-check-toml.md) | Goal, scope, acceptance, non-goals. |
...
| C3 | **`__init__.py` import-forwarding.** `python.md`: "**NEVER use import forwarding**". Existing [src/hooksmith/config/__init__.py](../../src/hooksmith/config/__init__.py) re-exports `load_config`, `HooksmithConfig`, etc. This is the documented public API in STY-0001. | Enforcement gate (would FAIL), API contract (would BREAK). | Two-path decision needed: **(a)** carve a documented exception for top-level package facade modules (recommended, matches stdlib convention and STY-0001's stated import path `from hooksmith.config import load_config`), or **(b)** rewrite the convention. Recording as a needed clarification, not silently bypassing. |
| C4 | **One-class-per-file rule vs schema cohesion.** `python.md`: "One class per file… filename must be the snake_case version of the class name." STY-0001 TSK-001 places four pydantic classes in one `schema.py`. | code-agent's schema_subagent. | Split into `src/hooksmith/config/source_spec.py`, `hook_def.py`, `group_def.py`, `hooksmith_config.py`. Keep `schema.py` only if approved as a barrel — but per the same convention, barrels are `__init__.py`-only. Recommendation: split. |
...
| E3 | Project venv (`.venv`) | Isolated env for editable install + dev deps. | env_python_subagent | `uv venv .venv --python 3.11 && uv pip install -e ".[dev]"`; `.venv/bin/python -c "import hooksmith"` exits 0. | `rm -rf .venv` |
...
| E5 | `mypy` strict-mode resolved | Static type gate (acceptance criterion). | env_python_subagent | `.venv/bin/mypy --version`; `mypy --strict src/hooksmith/config/` runs (currently passes against the stubs). | (no stop) |
...
| E8 | Rust 1.75+ + `cargo` + `clippy` + `rustfmt` | Required by check.toml hook set (`cargo-fmt-check`, `cargo-clippy`); STY-0001 doesn't touch Rust but the env lane verifies it because it's a downstream prerequisite. | env_rust_subagent | `cargo --version`, `cargo clippy --version`, `cargo fmt --version`. `cargo check --manifest-path hooksmith-rs/Cargo.toml` exits 0. | (no stop) |
| E9 | `maturin develop --release` (hooksmith-core) | Required so `from hooksmith import …` resolves the Rust wheel symbol set referenced in pyproject deps (`hooksmith-core>=0.1.0`). STY-0001 acceptance says "no runtime dependency on the Rust core", so the loader must not transitively import it — but the venv still needs to satisfy the wheel. | env_rust_subagent | `.venv/bin/python -c "import hooksmith_core; print(hooksmith_core.__file__)"` exits 0. | `rm .venv/lib/python3.11/site-packages/hooksmith_core*` |
...
| E12 | `hooksmith install` (this project's own pre-commit hooks) | Self-hosting check from check.toml — guarantees committed code passes the same hooks CI will. | env_hooks_subagent | `cat .git/hooks/pre-commit` includes `hooksmith`; `hooksmith run lint` against an empty staged set exits 0. | `git config --unset-all core.hooksPath`; or `hooksmith uninstall` once implemented. |
...
or `hooksmith-core` wheel is missing on this machine, the env lane reports
...
flowchart TD
    ...
    subgraph ENV ["Lane ENV — devops-agent (PREREQUISITE GATE)"]
        ...
        E_HOOKS[env_hooks_subagent<br/>hooksmith install]
    ...
    subgraph D ["Lane D — Implementation (code-agent)"]
        direction TB
        D1[schema_subagent<br/>source_spec.py · hook_def.py<br/>group_def.py · hooksmith_config.py]
...
| env_rust_subagent | devops-agent | E8, E9. | `hooksmith-rs/Cargo.toml`, `hooksmith-rs/pyproject.toml`. | `cargo check` green, `hooksmith_core` importable. | rust.md. |
| env_hooks_subagent | devops-agent | E12. | `check.toml`. | `.git/hooks/pre-commit` installed. | This repo's own check.toml. |
| env_watcher_subagent | devops-agent | E10, E11 background processes. | `tests/`, `src/`, `hooksmith-rs/src/`. | PID files + tailable logs under `.prompticorn/run/`. | general.md (file locations). |
...
| architect-agent | ADR-0001, PRD-0001, [docs/config/reference.md](../../docs/config/reference.md), current stubs in `src/hooksmith/config/`. | API sketch (file paths, public symbols, type signatures) committed to `planning/build-plans/0001-architecture-sketch.md`. **Locks the contract Lanes B/C/D consume.** | python.md (one-class-per-file; resolution to C4); ADR-0001. |

### 5.3 Lane B subagents (parent: test-agent)

| Subagent | Scope | Inputs | Outputs | Conventions |
|---|---|---|---|---|
| acceptance_subagent | Write acceptance tests that mirror STY-0001's 4 acceptance bullets. | architect sketch (5.2); STY-0001; `check.toml`. | `tests/integration/test_config_load_acceptance.py` containing: (i) load real repo `check.toml` parses to non-empty `HooksmithConfig`; (ii) `mypy --strict` shell-out passes; (iii) `pytest --cov` reports ≥ 90 % line coverage on `hooksmith.config`; (iv) no import of `hooksmith_core` from `hooksmith.config`. All tests must **fail red** until Lane D delivers. | skills `test-aaa-structure`, `test-coverage-categories`. Gherkin-style docstrings (resolution C7). |
...
| Subagent | Scope | Inputs | Outputs | Conventions |
|---|---|---|---|---|
| schema_subagent | Implement four pydantic models, one per file. | architect sketch; TSK-001. | `src/hooksmith/config/source_spec.py`, `hook_def.py`, `group_def.py`, `hooksmith_config.py`. No `pass`, no `TODO`, no `NotImplementedError`. | python.md (one class per file = C4 resolution; no module constants → use pydantic field defaults; no `setattr/getattr`). |
| loader_subagent | Implement `load_config`. | architect sketch; TSK-002; schema_subagent output. | `src/hooksmith/config/loader.py` using stdlib `tomllib` + pydantic. Synchronous. Side-effect free per STY-0001 Notes. | python.md (type hints mandatory; no nested defs; absolute imports). |

**`__init__.py` handling.** Per C3, the existing facade `from hooksmith.config import load_config` is preserved as the documented public API. The python.md "no import forwarding" rule is treated as not applicable to top-level package facade modules; this exception is recorded in the architecture sketch for enforcement-agent to honour.
...
| docs_subagent | document-agent | Write the usage section in `docs/config/reference.md`. | `hooksmith.config` public API; STY-0001 TSK-005. | New "Python API" section with a runnable snippet. | mkdocs Material conventions (existing site uses it). |
...
| Mandatory type hints; `mypy --strict src/hooksmith/config/` exits 0 | code-agent self-check; G2 aggregator runs mypy | G2 |
...
| Coverage gates (line ≥ 80 %, branch ≥ 70 %, function ≥ 90 %, statement ≥ 85 %, mutation ≥ 80 %) per `.prompticorn.yaml` | G2 aggregator runs `pytest --cov` and `mutmut run --paths-to-mutate=src/hooksmith/config/` | G2 |
...
test_config_package_does_not_import_hooksmith_core()
...
`mutmut run --paths-to-mutate=src/hooksmith/config/` against the new
...
|---|---|---|---|
| Filesystem → `tomllib` → pydantic → public dataclass | `python -c "from hooksmith.config import load_config; print(load_config('check.toml'))"` from a shell invocation inside the unit test runner, asserting non-empty `HooksmithConfig` with at least the 7 hooks and 3 groups from the repo `check.toml`. | G2 aggregator | orchestrator-agent |
| Public API import surface | `python -c "from hooksmith.config import load_config, HooksmithConfig, HookDef, SourceSpec, GroupDef"` exits 0. | G2 aggregator | orchestrator-agent |
| Type-checker integration | `mypy --strict src/hooksmith/config/ tests/unit/test_config_loader.py tests/unit/test_config_schema.py tests/integration/test_config_load_acceptance.py` exits 0. | G2 aggregator | orchestrator-agent |
| Self-hosted lint (hooksmith-on-hooksmith) | `hooksmith run lint` (which the env lane installed at E12) exits 0 against the staged diff. | G2 aggregator | orchestrator-agent |
...
- **Why a prerequisite env gate (G0) instead of running env work in
  ...
  produces tests that pass against `hooksmith.config` stubs and fail
...
NEW       src/hooksmith/config/source_spec.py                     (schema_subagent)
NEW       src/hooksmith/config/hook_def.py                        (schema_subagent)
NEW       src/hooksmith/config/group_def.py                       (schema_subagent)
NEW       src/hooksmith/config/hooksmith_config.py                (schema_subagent)
MODIFY    src/hooksmith/config/loader.py                          (loader_subagent — remove stub)
MODIFY    src/hooksmith/config/schema.py                          (delete or convert to back-compat re-export per C4 final decision)
MODIFY    src/hooksmith/config/__init__.py                        (update imports per file split)
...
- [ ] **G-4** Approve the one-class-per-file split (`source_spec.py`, `hook_def.py`, `group_def.py`, `hooksmith_config.py`) and removal of the bundled `schema.py`.
