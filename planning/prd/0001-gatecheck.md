---
id: PRD-0001
title: gatecheck — a modern pre-commit replacement
status: Draft
author: TBD
date: 2026-05-28
---
...
5. **Explainable caching.** `gatecheck cache why <hook>` produces a human-readable trace of the cache decision.
6. **Drop-in migration.** `gatecheck migrate` reads `.pre-commit-config.yaml` and writes a working `check.toml`.

## Non-goals

- We are not building a new linter, formatter, or test runner. gatecheck only *runs* hooks.
...
- **Performance**: p50 cold start under 10 ms; p50 cached `gatecheck run` on a 10-file commit under 250 ms.
- **Migration friction**: ≥ 90% of `gatecheck migrate` outputs run unmodified against the source repo's hook suite.
...
- Should `gatecheck` support Windows on day one, or as a fast follower? (Affects path handling in the Rust runner.)
- Is there a story for hook *authoring* (a `gatecheck-hooks` SDK), or do we treat any PyPI package as a hook host?
