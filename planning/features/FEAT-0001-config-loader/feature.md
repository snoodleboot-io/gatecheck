...
Parse a project's `check.toml` into a fully validated, typed in-memory model that the rest of `hooksmith` consumes. Surface schema errors with file/line context so users can fix their config without guessing.

## Why

Every other subsystem — sources, environments, runner, workspace — reads the result of this feature. Getting the model right (and the error messages humane) determines how the whole tool feels. See [PRD-0001 § Scope — Config](../../prd/0001-hooksmith.md#scope) and [ADR-0001](../../adr/0001-python-host-rust-core.md) for why the loader sits on the Python side.
...
- **Python API**: `from hooksmith.config import load_config; cfg: HooksmithConfig = load_config(Path("check.toml"))`.
...
- [ ] STY-0003 — Round-trip dump for `hooksmith migrate` output (not yet written)

## Acceptance

- A valid `check.toml` parses into a `HooksmithConfig` object whose fields match the documented schema.
