...
implement ``hooksmith.config`` to make them green. Do not mock the
...
def test_load_repo_check_toml_returns_non_empty_config() -> None:
    ...
    Then it returns a populated HooksmithConfig matching the file's contents.
    ...
    # Act
    cfg: HooksmithConfig = load_config(check_toml_path)
...
def test_mypy_strict_passes_on_config_package() -> None:
    """Given the hooksmith.config package and the project's strict mypy settings,
    When mypy --strict is run against src/hooksmith/config/,
    ...
    result: subprocess.CompletedProcess[str] = subprocess.run(
        [str(mypy_bin), "--strict", "src/hooksmith/config/"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
        check=False,
...
def test_config_package_public_imports() -> None:
    ...
    symbols: dict[str, object] = {
        ...
        "HooksmithConfig": HooksmithConfig,
        "HookDef": HookDef,
    ...
    assert isinstance(HooksmithConfig, type)
    assert isinstance(HookDef, type)
    assert isinstance(SourceSpec, type)
    assert isinstance(GroupDef, type)


def test_no_runtime_dependency_on_hooksmith_core() -> None:
    """Given a fresh Python subprocess,
    When every module of hooksmith.config is imported,
    Then hooksmith_core is not pulled into sys.modules as a side effect.
    ...
    probe: str = (
        "import sys; "
        "import hooksmith.config; "
        "import hooksmith.config.loader; "
        "import hooksmith.config.hooksmith_config; "
        "import hooksmith.config.hook_def; "
        "import hooksmith.config.group_def; "
        "import hooksmith.config.source_spec; "
        "assert 'hooksmith_core' not in sys.modules, sorted(sys.modules)"
    )
    ...
    # Assert
    assert result.returncode == 0, (
        f"hooksmith.config imports leaked hooksmith_core.\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )
...
def test_unit_test_coverage_meets_ninety_percent() -> None:
    ...
    When pytest is run with --cov-fail-under=90 against hooksmith.config,
    ...
    result: subprocess.CompletedProcess[str] = subprocess.run(
        [
            ...
            # GAT-48: discover_config is part of hooksmith.config; gate its tests too.
            "tests/unit/test_config_discovery.py",
            "--cov=hooksmith.config",
            "--cov-report=term",
            "--cov-fail-under=90",
            "-q",
        ],
