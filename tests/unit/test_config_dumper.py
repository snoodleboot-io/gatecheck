"""Unit tests for hooksmith.config.dumper.dump_config (STY-0003 / BUILD-0003).
...
from hooksmith.config import GroupDef, HookDef, HooksmithConfig, SourceSpec, load_config
from hooksmith.config.hook_def import HookWhen
...
def test_dump_creates_file(tmp_path: Path) -> None:
    """Given a minimal HooksmithConfig, When dump_config runs, Then the output
    file exists on disk."""
    # Arrange
...
def test_dump_empty_config_round_trips(tmp_path: Path) -> None:
    """Given a default HooksmithConfig(), When dumped then reloaded, Then the
    reloaded config equals the original."""
    # Arrange
    from hooksmith.config import dump_config

    cfg1 = HooksmithConfig()
    out = tmp_path / "check.toml"
...
def test_dump_sample_fixture_round_trips(sample_check_toml: Path, tmp_path: Path) -> None:
    """Given the bundled check.toml.sample loaded into a HooksmithConfig, When
...
def test_aot_count_matches_hooks(tmp_path: Path) -> None:
    ...
    cfg = HooksmithConfig(hook=hooks)
    out = tmp_path / "check.toml"
...
def test_sources_section_present_when_set(tmp_path: Path) -> None:
    ...
    cfg = HooksmithConfig(sources=SourceSpec(**{"default-registry": "https://pypi.org/simple"}))
    out = tmp_path / "check.toml"
...
def test_sources_absent_when_none(tmp_path: Path) -> None:
    ...
    cfg = HooksmithConfig(sources=None)
    out = tmp_path / "check.toml"
...
def test_sources_absent_when_empty_source_spec(tmp_path: Path) -> None:
    ...
    cfg = HooksmithConfig(sources=SourceSpec())
    out = tmp_path / "check.toml"
