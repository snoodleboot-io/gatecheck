"""Unit tests for `gatecheck cache clear` (STY-0027 / GAT-29).
...
def _seed_cache(cache_home: Path, *, keys: tuple[str, ...] = ("aaa",)) -> Path:
    """Create fake venv slots under ``<cache_home>/gatecheck/env-v1`` and return the root."""
    root = cache_home / "gatecheck"
    for key in keys:
...
def test_clear_reports_removed_environments(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / "gatecheck" / "env-v1" / "aaa").exists()
...
def test_dry_run_previews_without_deleting(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "gatecheck" / "env-v1" / "aaa").exists()
