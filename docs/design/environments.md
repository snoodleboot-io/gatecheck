...
How hooksmith turns `from = "pypi:ruff==0.4.9"` into a runnable tool, and why the cache
...
hooksmith's model is: a tool is a **package**. `ruff` is `pypi:ruff==0.4.9` — the same
...
`uv` is a host binary, not a Python dependency. hooksmith finds it via `HOOKSMITH_UV`,
then `PATH`, and if it's absent, **bootstraps** a pinned, checksum-verified copy into
the cache (`~/.cache/hooksmith/bin/uv`) — a one-time download from Astral's releases,
...
`HOOKSMITH_NO_BOOTSTRAP` forbids the download, turning a missing `uv` into a clear error
rather than an attempted fetch — the right behaviour for a locked-down runner.

## Integrity: every artifact hash

When `uv` installs a `pypi:` tool, hooksmith pins **every** known artifact hash for the
