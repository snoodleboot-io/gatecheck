...
```console
$ hooksmith cache why <HOOK>   # why is this hook a hit or a miss?
$ hooksmith cache clear        # reclaim the space
```
...
```console
$ hooksmith cache why <HOOK> [OPTIONS]
```
...
cache dir: /home/you/.cache/hooksmith/env-v1/c457c8e5…
```
...
`--all` additionally removes the `uv` that hooksmith bootstrapped for itself. Use it
when you want the cache directory genuinely empty; the next run re-downloads `uv`
unless one is already on your `PATH` or `HOOKSMITH_UV` is set.
...
!!! warning "There is no partial eviction"
    ...
    `clear` removes everything (or everything plus `uv`). hooksmith does not track
...
- [`hooksmith sync`](sync.md) — populate the cache deliberately.
